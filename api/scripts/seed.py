...
def seed_subregions(file_path="data/regions.json"):
    ...
    with open(file_path, "r", encoding="utf-8") as file:
        ...
        for subregion in data:
            region = Region.query.filter(Region.name == subregion["region"]).one()
...
def seed_countries(file_path="data/countries.json"):
    ...
    with open(file_path, "r", encoding="utf-8") as file:
        ...
        for country in data:
            ...
            region = Region.query.filter(Region.name == country["region"]).one()
            subregion = SubRegion.query.filter(
                SubRegion.name == country["subregion"]
            ).one()
            ...
            for alt_spelling in country["altSpellings"]:
                new_alt_spelling = AltCountry(
                    country_id=new_country, alt_name=alt_spelling
                )
        ...
        for ethnicity in data:
            if ethnicity not in [
                ...
                if isinstance(ethnicity, dict):
                    subregion = SubRegion.query.filter(
                        SubRegion.name == ethnicity["subregion"]
                    ).one()
...
def seed_cast_members(file_path="data/persons.json"):
    ...
    for person in person_data:
        ...
        ethnicity_list = get_ethnicity(new_person)
            ...
            for ethnicity in ethnicity_list:
                ethnicity_object = (
                    ...
                    .filter(
                        ...
                        | (AltEthnicity.alt_name == ethnicity)
                    )
                    .first()
                )

                if (
                    ethnicity_object is not None
                    and ethnicity_object.id not in add_ethnicities
                ):
                    add_ethnicities.add(ethnicity_object.id)
                    print(f"Adding {ethnicity_object.name} to {new_person.name}")
                    new_person.ethnicities.append(ethnicity_object)

        # Add race data
        print("Adding approximate race data...")
        add_race_ids = set()
        if len(new_person.ethnicities) == 0:
            ...
            for ethnicity in new_person.ethnicities:
                if ethnicity.region is not None:
                    approx_country = ethnicity
                else:
                    approx_country = Country.query.filter(
                        Country.demonym == ethnicity.name
                    ).first()
                ...
                if approx_country is not None:
                    ...
                    if approx_country.subregion.name in [
                        "Central America",
                        "South America",
                    ] or approx_country.name in ["Mexico", "Puerto Rico", "Cuba"]:
                    ...
                    print(f"Could not associate {ethnicity.name} with a country")
                    if ethnicity.name in [
                        "English",
                        "Cornish",
                        "Scottish",
                        "Welsh",
                        ...
                        add_race_ids.add(white.id)
                    if (
                        ethnicity.name == "African American"
                        or ethnicity.name == "African"
                    ):
                        black = Race.query.filter(Race.short == "BLK").one()
                        add_race_ids.add(black.id)
