 ...
 */
export const parseGenders = (movieCast: Cast[] | undefined) => {
	if (!movieCast) {
		return undefined;
	}
 ...
 */
export const parseRace = (movieCast: Cast[] | undefined) => {
	if (!movieCast) return undefined;
	if (movieCast.length === 0) return [];
	...
	return movieCast
		...
		.reduce((acc, race) => {
			...
			return listRaceData;
		}, listRaceData) as ChartData[];
};

/**
 * Parses ethnicity data from a list of cast members.
 */
export const parseEthnicity = (movieCast: Cast[] | undefined) => {
	if (!movieCast) return undefined;
	if (movieCast.length === 0) return [];
	return movieCast
